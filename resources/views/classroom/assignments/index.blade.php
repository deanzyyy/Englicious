...
<div class="container">
    <div class="py-10">
        <div class="flex justify-between items-center mb-6">
            ...
            <a href="{{ route('classroom.assignments.create', $classroom->name) }}" class="px-4 py-2 bg-gradient-to-r from-pink-500 to-orange-500 text-white rounded-lg hover:opacity-90 transition-all text-sm font-medium uppercase tracking-wider">Create Assignment</a>
            @endif
        </div>
        <div class="space-y-5">
            @forelse($assignments as $assignment)
                <div class="card w-full h-auto p-5 bg-[#211F27] rounded-lg flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        <h2 class="text-white font-bold text-lg mb-0">{{ $assignment->title }}</h2>
                    ...
                    <a href="{{ route('classroom.assignments.show', [$classroom->name, $assignment->id]) }}" class="rounded-lg px-4 py-2 text-gray-300 border border-transparent hover:border-pink-500 hover:text-pink-500 hover:bg-pink-950/20 transition-all duration-300 uppercase tracking-wider text-sm font-medium">Open</a>
                </div>
            @empty
