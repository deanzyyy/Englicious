    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="bg-[#211F27] rounded-lg overflow-hidden shadow-lg border border-pink-500/20">
            ...
            <div class="p-6 border-b border-gray-700">
                ...
                <p class="text-gray-400">{{ $exercise->title }} - {{ $classroom->name }}</p>
            </div>
            ...
            <div class="p-6 bg-gradient-to-r from-pink-500/10 to-orange-500/10">
                <div class="flex items-center justify-between mb-6">
                    <div>
                        ...
                        <p class="text-gray-400">Completed on {{ $submission->created_at->format('F j, Y, g:i a') }}</p>
                    </div>
                    <div class="text-right">
                        <div class="text-4xl font-bold text-pink-500">{{ $recalculatedScore ?? $submission->score }}%</div>
                        @php
                        ...
                        @endphp
                        <div class="mt-2 text-lg font-bold {{ $motivasiColor }}">{{ $motivasi }}</div>
                            ...
                            <div class="flex items-start space-x-4">
                                <div class="flex-shrink-0">
                                    @if($question->type === 'optional')
                                        @if(isset($submission->answers[$question->id]) && $submission->answers[$question->id] == $question->correct_answer)
                                ...
                                <div class="flex-1">
                                    ...
                                    <p class="text-white mb-3">{{ $question->question_text }}</p>
                                        ...
                                        <div class="grid grid-cols-2 gap-3">
                                            @foreach($question->options as $optionIndex => $option)
                                                <div class="flex items-center space-x-2">
                                                    ...
                                                    @elseif(isset($submission->answers[$question->id]) && $submission->answers[$question->id] == $optionIndex)
                                                        <span class="text-red-500">✗</span>
                                                    @endif
                                                    <span class="text-gray-400 {{ $optionIndex == $question->correct_answer ? 'text-green-500' : '' }}">
                                                        {{ $option }}
                                                    </span>
                                                </div>
                                            @endforeach
                                    ...
                                    @elseif($question->type === 'essay')
                                        <div class="mt-2">
                                            <div class="mb-1 text-gray-400">Your Answer:</div>
                                            <div class="bg-[#18161d] text-white rounded-lg p-3">{{ $submission->answers[$question->id] ?? '-' }}</div>
                                            <div class="mt-2">
                                                @if(isset($submission->essay_scores[$question->id]))
                                                    <span class="text-green-400 font-semibold">Score: {{ $submission->essay_scores[$question->id] }}</span>
                                                    @if(isset($submission->essay_comments[$question->id]))
                                                        <div class="text-gray-400 mt-1">Comment: {{ $submission->essay_comments[$question->id] }}</div>
                                                    @endif
                                                @else
                                                    <span class="text-orange-400 font-semibold">Waiting for review</span>
                                                @endif
                                            </div>
                                        </div>
                                    @endif
