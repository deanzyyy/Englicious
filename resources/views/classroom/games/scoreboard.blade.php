...
<div class="mt-8">
    <div class="mb-8">
        <h2 class="text-2xl font-bold text-white">{{ $game->name }} - Final Results</h2>
    ...
    <div class="bg-gradient-to-r from-yellow-500/20 to-orange-500/20 rounded-lg p-6 mb-6 border border-yellow-500/40">
        <div class="text-center">
            ...
            <h3 class="text-2xl font-bold text-yellow-400 mb-2">
                🏆 Winner: {{ ($scores->first()->player->team) ? $scores->first()->player->team->name : ($scores->first()->player->student_name ?? $scores->first()->player->user->name ?? 'Unknown') }}
            </h3>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
        <div class="bg-[#211F27] rounded-lg p-6 border border-pink-500/20">
            <div class="text-center">
                <div class="text-3xl font-bold text-pink-400">{{ $scores->max('score') }}</div>
                <div class="text-gray-400 text-sm">Highest Score</div>
            </div>
        </div>
        <div class="bg-[#211F27] rounded-lg p-6 border border-pink-500/20">
            <div class="text-center">
                <div class="text-3xl font-bold text-blue-400">{{ round($scores->avg('score'), 1) }}</div>
                <div class="text-gray-400 text-sm">Average Score</div>
            </div>
        </div>
        <div class="bg-[#211F27] rounded-lg p-6 border border-pink-500/20">
            <div class="text-center">
                <div class="text-3xl font-bold text-green-400">{{ $scores->count() }}</div>
