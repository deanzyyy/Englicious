    ...
    <div class="ml-10 p-10">
        ...
        <div class="flex justify-between items-center mb-8">
            <div>
                ...
                <p class="text-lg text-gray-400 mt-2">Total: {{ $total_exercises }} {{ Str::plural('Exercise', $total_exercises) }}</p>
            </div>
            ...
            <div class="space-y-4">
                @foreach($exercises as $category => $topicGroups)
                    <div class="exercise-category" data-category="{{ $category }}">
                        ...
                        <!-- Topics List -->
                        <div class="space-y-4">
                                @foreach($topicGroups as $topicName => $topicExercises)
                                        ...
                                        <div class="p-4">
                                            <div class="flex items-center justify-between">
                                                <div class="flex items-center space-x-3">
                                                    ...
                                                    <span class="text-gray-400">({{ $topicExercises->count() }} {{ Str::plural('Exercise', $topicExercises->count()) }})</span>
                                                </div>
                                                    <button onclick="toggleSubtopics('{{ str_replace(' ', '_', $topicName) }}')" class="text-gray-400 hover:text-white transition-colors">
                                                        <i id="icon-{{ str_replace(' ', '_', $topicName) }}" class="fi fi-rr-angle-small-down transform transition-transform duration-200"></i>
                                                    </button>
                                            </div>
                                        </div>

                                        <div id="subtopics-{{ str_replace(' ', '_', $topicName) }}" class="hidden border-t border-gray-700">
                                            <div class="p-4 space-y-4">
                                                ...
                                                @foreach($subtopicGroups as $subtopicName => $exercises)
                                                    <div class="ml-8 p-4 bg-[#1a1a1f] rounded-lg">
                                                        <div class="flex items-center justify-between mb-3">
                                                            ...
                                                            <span class="text-gray-400 text-sm">{{ $exercises->count() }} {{ Str::plural('Exercise', $exercises->count()) }}</span>
                                                        </div>
                                                                ...
                                                                <div class="flex items-center justify-between py-2 px-4 bg-[#211F27] rounded-lg hover:bg-[#2a2833] transition-all duration-300">
                                                                    <div class="flex-grow">
                                                                        ...
                                                                        <div class="flex items-center space-x-2 mt-2">
                                                                            ...
                                                                            @if($exercise->questions_count)
                                                                                <span class="text-xs text-gray-500">
                                                                                    <i class="fi fi-rr-interrogation mr-1"></i>
                                                                                    {{ $exercise->questions_count }} {{ Str::plural('Question', $exercise->questions_count) }}
                                                                                </span>
                                                                            @endif
                                                                        </div>
                                                                    </div>
                                                                    <div class="flex items-center space-x-2">
                                                                        <button onclick="viewExercise({{ $exercise->id }})"
        ...
        @else
            <div class="bg-[#211F27] rounded-lg shadow-lg border border-white/20 p-8 text-center">
                ...
                <p class="text-gray-400 mb-4">
                    @if(request('category'))
                        No exercises found in the {{ request('category') }} category.
                    @else
        ...
        <div id="addTopicModal" class="modal hidden fixed inset-0 bg-black bg-opacity-10 backdrop-blur-sm z-50 flex items-center justify-center">
            <div class="modal-content bg-[#211F27] rounded-lg shadow-xl border border-pink-500/20 w-full max-w-md mx-4">
                <div class="p-6">
                    ...
                    <form action="{{ route('topics.store') }}" method="POST" class="space-y-4" id="addTopicForm">
                    @csrf
                        <!-- Category Selection -->
                        <div>
