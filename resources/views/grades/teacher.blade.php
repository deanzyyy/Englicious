...
<div class="container mx-auto px-4 py-8">
    <div class="bg-[#211F27] rounded-lg p-6 shadow-lg">
        <div class="flex items-center justify-between mb-6">
            <h2 class="text-2xl font-bold text-pink-500">{{ $classroom->name }} - Student Grades</h2>
                            ...
                            <tr class="hover:bg-[#32323A] transition-colors">
                                        ...
                                        <div class="flex-shrink-0 h-10 w-10">
                                            <div class="h-10 w-10 rounded-full bg-gradient-to-r from-pink-500 to-orange-500 flex items-center justify-center">
                                                <span class="text-white font-semibold text-sm">
                                                    {{ strtoupper(substr($gradeData['student']->name, 0, 2)) }}
                                                </span>
                                            </div>
                                        </div>
                                        <div class="ml-4">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm">
                                    <span class="text-transparent bg-clip-text bg-gradient-to-r from-pink-500 to-orange-500 font-medium">
                                        {{ number_format($gradeData['avgExercise'], 2) }}%
                                    </span>
                                    <div class="text-xs text-gray-400">
                                        {{ $gradeData['exerciseSubmissions']->count() }} exercises
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm">
                                    @if($gradeData['avgAssignment'] !== null)
                                        <span class="text-transparent bg-clip-text bg-gradient-to-r from-pink-500 to-orange-500 font-medium">
                                            {{ number_format($gradeData['avgAssignment'], 2) }}%
                                        </span>
                                        <div class="text-xs text-gray-400">
                                            {{ $gradeData['assignmentSubmissions']->whereNotNull('grade')->count() }} graded
                                        </div>
                                    @else
                                        <span class="text-gray-500">Menunggu penilaian</span>
                                        <div class="text-xs text-gray-400">
                                            {{ $gradeData['assignmentSubmissions']->count() }} submitted
                                        </div>
                                    @endif
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm">
                                    @if($gradeData['finalScore'] !== null)
                                        <span class="text-transparent bg-clip-text bg-gradient-to-r from-pink-500 to-orange-500 font-bold text-lg">
                                            {{ number_format($gradeData['finalScore'], 2) }}%
                                        </span>
                                    @else
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm">
                                    <button onclick="viewStudentDetails('{{ $gradeData['student']->id }}', '{{ $gradeData['student']->name }}')"
...
<script>
    function viewStudentDetails(studentId, studentName) {
        ...
        fetch(`/classroom/{{ $classroom->name }}/grades/student/${studentId}`)
    ...
    document.getElementById('grade-assignment-form').addEventListener('submit', function(e) {
        ...
        fetch(`/classroom/{{ $classroom->name }}/grades/assignment/${submissionId}`, {
            method: 'POST',
            headers: {
