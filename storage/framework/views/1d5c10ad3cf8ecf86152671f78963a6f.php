...
<?php $component->withAttributes([]); ?>
    <div class="min-h-screen bg-[#101014] p-8">
        <div class="max-w-7xl mx-auto">
            ...
            <div class="flex justify-between items-center mb-8">
                <div>
                    <h1 class="text-4xl font-bold text-white"><?php echo e($classroom->name); ?></h1>
                ...
                <div class="flex gap-4">
                    ...
                    <a href="<?php echo e(route('attendance.history', $classroom)); ?>" 
                    ...
                    <a href="<?php echo e(route('attendance.export-pdf', $classroom)); ?>" 
                       class="px-6 py-3 border-2 border-pink-500 text-white rounded-lg hover:bg-gradient-to-r from-pink-500 to-orange-500 hover:border-transparent transition-all flex items-center gap-2">
                    ...
                    <tbody class="divide-y divide-gray-700">
                        <?php $__empty_1 = true; $__currentLoopData = $students; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $student): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <tr class="hover:bg-gray-800/50">
                                <td class="py-4 px-6 text-white"><?php echo e($index + 1); ?></td>
                                <td class="py-4 px-6 text-white"><?php echo e($student->name); ?></td>
                                <td class="py-4 px-6">
                                    <div class="flex justify-center gap-2">
                                        ...
                                        <button onclick="updateAttendance('<?php echo e($student->id); ?>', 'present')" 
                                                class="w-10 h-10 rounded-full <?php echo e($status === 'present' ? 'bg-green-500' : 'bg-gray-700 hover:bg-green-500/20'); ?> flex items-center justify-center transition-colors"
                                                title="Present">
                                            <i class="fi fi-rr-check text-white"></i>
                                        </button>
                                        <button onclick="updateAttendance('<?php echo e($student->id); ?>', 'late')" 
                                                class="w-10 h-10 rounded-full <?php echo e($status === 'late' ? 'bg-blue-500' : 'bg-gray-700 hover:bg-blue-500/20'); ?> flex items-center justify-center transition-colors"
                                                title="Late">
                                            <i class="fi fi-rr-time-forward text-white"></i>
                                        </button>
                                        <button onclick="updateAttendance('<?php echo e($student->id); ?>', 'excused')" 
                                                class="w-10 h-10 rounded-full <?php echo e($status === 'excused' ? 'bg-yellow-500' : 'bg-gray-700 hover:bg-yellow-500/20'); ?> flex items-center justify-center transition-colors"
                                                title="Excused">
                                            <i class="fi fi-rr-note text-white"></i>
                                        </button>
                                        <button onclick="updateAttendance('<?php echo e($student->id); ?>', 'absent')" 
                                                class="w-10 h-10 rounded-full <?php echo e($status === 'absent' ? 'bg-red-500' : 'bg-gray-700 hover:bg-red-500/20'); ?> flex items-center justify-center transition-colors"
                                                title="Absent">
                                ...
                                <td class="py-4 px-6">
                                    <div class="flex justify-center gap-2">
                                        <button onclick="editStudent('<?php echo e($student->id); ?>', '<?php echo e($student->name); ?>')"
    ...
    <script>
        ...
        function confirmDelete() {
            ...
            fetch(`/classroom/<?php echo e($classroom->id); ?>/students/${studentToDelete}`, {
                method: 'DELETE',
                headers: {
        ...
        document.getElementById('addStudentForm').addEventListener('submit', function(e) {
            ...
            fetch('<?php echo e(route("classroom.students.add", $classroom->name)); ?>', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
        ...
        document.getElementById('editStudentForm').addEventListener('submit', function(e) {
            ...
            fetch(`/classroom/<?php echo e($classroom->name); ?>/students/${studentId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
...
<?php endif; ?> <?php /**PATH D:\Englicious\Englicious\resources\views/attendance/show.blade.php ENDPATH**/ ?>