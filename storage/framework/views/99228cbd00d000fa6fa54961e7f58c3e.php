...
<div class="container">
    <div class="py-10">
        ...
        <div class="list-exercises w-full space-y-5 pt-5">
            <?php if($classroom->exercises->isEmpty()): ?>
            ...
            <?php else: ?>
                <?php $__currentLoopData = $classroom->exercises; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $exercise): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div id="exercise-card-<?php echo e($exercise->id); ?>" class="group card w-full h-auto p-5 bg-[#211F27] rounded-lg flex items-center justify-between relative overflow-hidden transition-all duration-300 hover:bg-pink-950/20 before:content-[''] before:absolute before:top-0 before:left-0 before:w-1 before:h-full before:bg-pink-500/0 hover:before:bg-pink-500 after:content-[''] after:absolute after:bottom-0 after:left-0 after:w-full after:h-[2px] after:bg-gradient-to-r after:from-pink-500/50 after:to-transparent">
                        <div class="judul">
                            <h1 class="text-white font-bold text-lg group-hover:text-pink-500 transition-colors"><?php echo e($exercise->title); ?></h1>
                            <p class="text-gray-400 group-hover:text-pink-300 transition-colors"><?php echo e($exercise->category); ?> - <?php echo e($exercise->topic->name); ?></p>
                        </div>
                        <div class="w-auto">
                            <p class="text-gray-400 group-hover:text-pink-300 transition-colors"><?php echo e($exercise->created_at->format('d-M-Y')); ?></p>
                        </div>
                        <div class="flex space-x-3">
                            <?php if($exercise->is_file_upload): ?>
                                <a href="<?php echo e(route('preview.file', $exercise->id)); ?>" 
                            ...
                            <?php else: ?>
                                <a href="<?php echo e(route('classroom.exercise.take', ['className' => $classroom->name, 'exerciseId' => $exercise->id])); ?>" 
                                   class="rounded-lg px-4 py-2 text-gray-300 border border-transparent hover:border-pink-500 hover:text-pink-500 hover:bg-pink-950/20 transition-all duration-300 uppercase tracking-wider text-sm font-medium">
                                    Kerjakan
                                </a>
                            <?php endif; ?>
                            <?php if(auth()->user()->role !== 'student'): ?>
                                <a href="<?php echo e(route('classroom.exercise.review', ['className' => $classroom->name, 'exerciseId' => $exercise->id])); ?>"
                                ...
                                <button type="button" onclick="confirmDelete(<?php echo e($exercise->id); ?>)" 
...
async function deleteExercise(exerciseId) {
    try {
        const response = await fetch(`/classroom/<?php echo e($classroom->id); ?>/exercises/${exerciseId}`, {
            method: 'DELETE',
            headers: {
...
<?php echo $__env->make('classroom.show', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\Englicious\Englicious\resources\views/classroom/exercises.blade.php ENDPATH**/ ?>