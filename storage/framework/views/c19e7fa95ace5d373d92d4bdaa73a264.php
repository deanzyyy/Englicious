...
<div class="mt-8">
    <div class="mb-8">
        <h2 class="text-2xl font-bold text-white"><?php echo e($game->name); ?></h2>
        <p class="text-gray-400 text-lg mt-2">Playing: <?php echo e($game->exercise->title); ?></p>
        ...
        <?php if(auth()->user()->role === 'student'): ?>
            ...
            <div class="bg-[#211F27] rounded-lg p-6 mb-6 border border-pink-500/20">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
                    ...
                    <div>
                        <div class="text-2xl font-bold text-green-500" id="totalQuestions"><?php echo e($game->exercise->questions->count()); ?></div>
    ...
    <?php else: ?>
        ...
        <?php if(auth()->user()->role === 'student'): ?>
            ...
            <div class="mt-6 flex gap-4">
                ...
                <?php if($game->status === 'draft' && $game->players->count() > 0): ?>
...
let questions = <?php echo json_encode($game->exercise->questions, 15, 512) ?>;
let players = <?php echo json_encode($game->players, 15, 512) ?>;
let scores = {};
let timer;
let timeLeft = 15;
let hasAnswered = false;
let gameType = '<?php echo e($game->type); ?>';
let userRole = '<?php echo e(auth()->user()->role); ?>';
let isOnlineTeacher = <?php echo e(($game->type === 'online' && auth()->user()->role !== 'student') ? 'true' : 'false'); ?>;
...
function sendStudentScore(points) {
    fetch('<?php echo e(route("classroom.games.score", ["className" => $classroom->name, "game" => $game->id])); ?>', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
            player_id: <?php echo e(auth()->user()->id); ?>,
            score: points,
...
function finishGame() {
    clearInterval(timer);
    window.location.href = '<?php echo e(route("classroom.games.scoreboard", ["className" => $classroom->name, "game" => $game->id])); ?>';
}
...
function loadStudents() {
    ...
    const students = <?php echo json_encode($classroom->users()->where('role', 'student')->get(), 512) ?>;
...
<?php echo $__env->make('classroom.show', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\Englicious\Englicious\resources\views/classroom/games/play.blade.php ENDPATH**/ ?>